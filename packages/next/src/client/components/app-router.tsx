...
import { legacyUrgentBFCacheRestore, traverse } from './navigator'
...
import { takeHistoryOwnership, writeHistory } from './history-handlers'
...
function Router({
  ...
  useEffect(() => {
    const handoff = takeHistoryOwnership()
    if (handoff.kind === 'reload') {
      window.location.reload()
    } else if (handoff.kind === 'restore') {
      traverse(window.location.href, handoff.historyState)
    }
  }, [])

...
async fn project_server_hmr_snapshot_operation(
    ...
    let chunks = project.server_hmr_chunks().await?;
    let version = AggregateHmrVersion::from_chunks(&chunks).await?.await?;
...
async fn server_hmr_snapshot_with_effects_operation(
    ...
    let snapshot_op = project_server_hmr_snapshot_operation(project);
...
pub async fn project_get_server_hmr_update(
    ...
    // Diffing needs a task context but must not remain active.
    let update = turbo_tasks
        .run_once(async move {
            ...
            mark_top_level_task();
            let snapshot_op = server_hmr_snapshot_with_effects_operation(project);
            let read =
